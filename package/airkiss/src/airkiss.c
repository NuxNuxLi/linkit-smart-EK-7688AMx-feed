...
{
    signal(SIGPIPE, &exit_airkiss);
    signal(SIGINT,  &exit_airkiss);
    signal(SIGTERM, &exit_airkiss);
    signal(SIGSEGV, &exit_airkiss);
    signal(SIGBUS,  &exit_airkiss);
}
...
int main(int argc, char* argv[])
{
    ...
    {
	memset(data, 0x00, RX_BUF_SIZE);
	if(_GO_) strcpy(data,"easeresult");
	strcpy(wrq.ifr_name, "ra0");
	wrq.u.data.length = RX_BUF_SIZE;
	wrq.u.data.pointer = data;
	wrq.u.data.flags = 0;
	...
	else
	{
	    ...
	    printf("---utility.main(%d)\n", mp->length);
			...
			{
			printf("%-8.16s, data: %s, len: %d \n", __FUNCTION__, mp->data, mp->length);
#if 1
